...
✅ GOOD: "The authentication module at `src/auth/login.ts:45-78` handles login by calling `validateCredentials()` at line 52"

### Use Exact Code References

❌ BAD: "There's error handling in the user service"
✅ GOOD: "Error handling in `src/services/user.service.ts:123-130` catches `UserNotFoundError` and returns a 404 response"

### Document Relationships Precisely

❌ BAD: "The controller calls the service"
✅ GOOD: "The `UserController.getUser()` at `src/controllers/user.ts:34` calls `UserService.findById()` at `src/services/user.ts:89`"
...
📄 `context/research/2025-12-26-notification-system.md`

### Summary
The notification system uses a pub/sub pattern with a central NotificationService...

### Key Discoveries
- Entry point at `src/services/notification.service.ts:45`
- Templates stored in `src/templates/notifications/`
- Email delivery via `src/integrations/sendgrid.ts:23`

...
1. **Step 1**: `file.ts:XX` - [What happens]
2. **Step 2**: `other.ts:YY` - [What happens]
3. **Fault Point**: `faulty.ts:ZZ` - [Where behavior deviates]
...
Start with the fault: "The bug occurs at `file.ts:XX` where [fault behavior]"
...
**Files Affected**:
- `file1.ts:XX-YY` - [Change description]
- `file2.ts:ZZ` - [Change description]
...
✅ GOOD: "Add null check at `auth.ts:45` before calling `user.getName()`, return early with `AuthError` if user is null"

❌ BAD: "Improve error handling"
✅ GOOD: "Wrap API call in `auth.ts:78-82` with try/catch, catch `TimeoutError` specifically, retry with exponential backoff (max 3 attempts)"
...
**Fault**: Application crashes at `user-service.ts:123` with `Cannot read property 'name' of null`

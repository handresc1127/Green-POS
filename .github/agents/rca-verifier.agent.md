...
|------|-----------|--------|-----------------|---------|
| Entry | `src/auth.ts:45` | ✅ | ✅ | ✅ PASS |
| Step 2 | `src/db.ts:120` | ✅ | ⚠️ Line off by 3 | ⚠️ MINOR |
| Fault | `src/handler.ts:78` | ✅ | ✅ | ✅ PASS |
...
|----------|--------|--------|
| `file.ts:45` | `file.ts:48` (function moved) | Low |
| `handler.ts:78` | File not found | 🔴 Critical |
```
...
#### 1. `functionName` in `path/to/file.ts`

**Usages Found:** [X] locations

| Location | Impact Risk | Notes |
|----------|-------------|-------|
| `caller1.ts:45` | Low | Only uses return value |
| `caller2.ts:120` | ⚠️ Medium | Depends on error handling |
| `test.spec.ts:30` | Low | Test mock |
